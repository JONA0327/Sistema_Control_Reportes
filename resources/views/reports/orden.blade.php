<x-app-layout>
    ...
    @php
        $tabConErrores = $errors->hasAny(['mecanico_id', 'tipo_atencion', 'falla_confirmada', 'subsistemas', 'codigo_falla', 'diagnostico', 'proveedor_externo', 'proveedor_externo_user_id', 'folio_proveedor', 'motivo_externo', 'fecha_promesa_entrega'])
            ? 'diagnostico'
            : ($errors->hasAny(['accion', 'pieza', 'notas', 'evidencia', 'item_id', 'quantity_used']) ? 'evidencia' : 'detalle');
    @endphp

    <div x-data="{ activeTab: '{{ $tabConErrores }}', tipoAtencion: '{{ old('tipo_atencion', $orden->tipo_atencion) }}' }" class="max-w-3xl">

        {{-- Pestañas --}}
        <div class="flex gap-2 mb-4 overflow-x-auto">
            <button type="button" @click="activeTab = 'detalle'"
                    class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all flex-shrink-0"
                    :class="activeTab === 'detalle' ? 'brand-gradient text-white shadow-lg shadow-red-950/40' : 'bg-gray-800/60 border border-gray-700/50 text-gray-400 hover:text-white hover:border-gray-600'">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                Detalles del reporte
            </button>
            <button type="button" @click="activeTab = 'diagnostico'"
                    class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all flex-shrink-0"
                    :class="activeTab === 'diagnostico' ? 'brand-gradient text-white shadow-lg shadow-red-950/40' : 'bg-gray-800/60 border border-gray-700/50 text-gray-400 hover:text-white hover:border-gray-600'">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button type="button" @click="activeTab = 'evidencia'"
                    class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all flex-shrink-0"
                    :class="activeTab === 'evidencia' ? 'brand-gradient text-white shadow-lg shadow-red-950/40' : 'bg-gray-800/60 border border-gray-700/50 text-gray-400 hover:text-white hover:border-gray-600'">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                </svg>
                Refacciones y evidencia
                @if($orden->piezas->count() > 0)
                    <span class="text-xs px-1.5 py-0.5 rounded-full" :class="activeTab === 'evidencia' ? 'bg-black/20' : 'bg-gray-700'">{{ $orden->piezas->count() }}</span>
                @endif
            </button>
        </div>
        ...
        {{-- ============ TAB 1: Detalles del reporte ============ --}}
        <div x-show="activeTab === 'detalle'" x-cloak class="space-y-4">
            <div class="bg-gray-800/40 border border-gray-700/40 rounded-2xl p-5">
                ...
                @if($report->frecuencia || !empty($report->condiciones) || !empty($report->sintomas))
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs mb-3 px-3.5 py-3 bg-gray-900/40 border border-gray-800 rounded-xl">
                        @if($report->frecuencia)
                            <div>
                                <p class="text-gray-500 uppercase tracking-wider mb-1">Frecuencia</p>
                                <p class="text-gray-200">{{ \App\Models\Report::FRECUENCIAS[$report->frecuencia] ?? '—' }}</p>
                            </div>
                        @endif
                        @if(!empty($report->condiciones))
                            <div>
                                <p class="text-gray-500 uppercase tracking-wider mb-1">Cuándo ocurre</p>
                                <p class="text-gray-200">{{ collect($report->condiciones)->map(fn($c) => \App\Models\Report::CONDICIONES[$c] ?? $c)->implode(', ') }}</p>
                            </div>
                        @endif
                        @if(!empty($report->sintomas))
                            <div>
                                <p class="text-gray-500 uppercase tracking-wider mb-1">Se percibe</p>
                                <p class="text-gray-200">{{ collect($report->sintomas)->map(fn($s) => \App\Models\Report::SINTOMAS[$s] ?? $s)->implode(', ') }}</p>
                            </div>
                        @endif
                    </div>
                @endif
                ...
                @if($report->photos->count() > 0)
                    <div class="mb-1">
                        <p class="text-xs text-gray-500 uppercase tracking-wider mb-1.5">Fotos del operador</p>
                        <div class="flex flex-wrap gap-2">
                            @foreach($report->photos as $photo)
                                <a href="{{ Storage::url($photo->evidence_path) }}" target="_blank">
                                    <img src="{{ Storage::url($photo->evidence_path) }}" class="w-20 h-20 object-cover rounded-lg border border-gray-700/50 hover:border-gray-500 transition-colors"/>
                                </a>
                            @endforeach
                        </div>
                    </div>
                ...
                @if($report->videos->count() > 0)
                    <div class="mt-3">
                        <p class="text-xs text-gray-500 uppercase tracking-wider mb-1.5">Videos del operador</p>
                        <div class="flex flex-wrap gap-2">
                            @foreach($report->videos as $video)
                                <video src="{{ Storage::url($video->evidence_path) }}" class="w-36 h-24 object-cover rounded-lg border border-gray-700/50" controls></video>
                            @endforeach
                        </div>
                    </div>
                @endif
            ...
            <button type="button" @click="activeTab = 'diagnostico'"
                    class="w-full flex items-center justify-center gap-2 px-5 py-3 bg-gray-800/40 border border-gray-700/40 hover:border-gray-600 rounded-2xl text-sm font-medium text-gray-300 hover:text-white transition-all">
        ...
        {{-- ============ TAB 2: Diagnóstico y atención ============ --}}
        <div x-show="activeTab === 'diagnostico'" x-cloak class="space-y-4">
            <form method="POST" action="{{ route('reports.orden.update', $report) }}">
                @csrf
                @method('PUT')

                <div class="bg-gray-800/40 border border-gray-700/40 rounded-2xl divide-y divide-gray-700/40">

                    {{-- 1. Recepción y responsable --}}
                    <div class="px-6 py-5">
                        <h2 class="text-sm font-semibold text-white mb-1 flex items-center gap-2">
                            ...
                            Recepción y responsable
                        </h2>
                        <p class="text-xs text-gray-600 mb-4">¿Quién atiende esta unidad y qué tipo de atención requiere?</p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div class="sm:col-span-2">
                                <label for="mecanico_id" class="block text-xs font-medium text-gray-400 mb-1.5">
                                    Mecánico / Técnico responsable <span class="text-red-500">*</span>
                                </label>
                            ...
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-medium text-gray-400 mb-2">
                                    ¿Dónde se va a atender la unidad? <span class="text-red-500">*</span>
                                </label>
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    @foreach([
                                        'taller_interno' => ['Taller interno', 'Se repara aquí mismo, con el equipo del taller.'],
                                        'taller_externo' => ['Taller externo', 'Se envía a un taller o agencia especializada.'],
                                        'auxilio_vial'   => ['Auxilio vial', 'Rescate o atención en ruta; la unidad no puede llegar al taller.'],
                                    ] as $val => [$label, $desc])
                                    <label class="relative cursor-pointer">
                                        ...
                                        <div class="px-3.5 py-3 rounded-xl border border-gray-700 bg-gray-900/40 peer-checked:border-red-500 peer-checked:bg-red-600/10 transition-all h-full">
                    ...
                    <div class="px-6 py-5">
                        ...
                        <p class="text-xs text-gray-600 mb-4">Lo que encontraste al revisar la unidad. Esto queda registrado en el historial del reporte.</p>

                        <div class="space-y-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-400 mb-2">¿Pudiste confirmar la falla que reportó el operador?</label>
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    @foreach([
                                        'si'           => ['Sí', 'Se comprobó la falla en el taller.'],
                                        'no'           => ['No', 'No se pudo reproducir lo que reportó el operador.'],
                                        'intermitente' => ['Intermitente', 'Aparece a veces; no se replicó de forma constante.'],
                                    ] as $val => [$label, $desc])
                                    <label class="relative cursor-pointer">
                                        ...
                                        <div class="px-3 py-2.5 rounded-xl border border-gray-700 bg-gray-900/40 peer-checked:border-red-500 peer-checked:bg-red-600/10 transition-all h-full">
                                            <span class="text-xs font-semibold text-gray-200">{{ $label }}</span>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-400 mb-1.5">Sistema real afectado</label>
                                <p class="text-xs text-gray-600 mb-2">Puede diferir de lo que marcó el operador — indica el sistema exacto según tu diagnóstico.</p>
                            ...
                            <div>
                                <label for="codigo_falla" class="block text-xs font-medium text-gray-400 mb-1.5">
                                    Código de falla del escáner <span class="text-gray-600 font-normal">(opcional)</span>
                                </label>
                                <p class="text-xs text-gray-600 mb-1.5">Si usaste escáner/diagnóstico electrónico, anota el código que arrojó.</p>
                            ...
                            <div>
                                <label for="diagnostico" class="block text-xs font-medium text-gray-400 mb-1.5">
                                    ¿Qué encontraste y por qué pasa la falla?
                                </label>
                    ...
                    <div class="px-6 py-5" x-show="tipoAtencion === 'taller_externo'" x-cloak>
                        <h2 class="text-sm font-semibold text-white mb-1 flex items-center gap-2">
                            ...
                            Canalización a taller externo
                        </h2>
                        <p class="text-xs text-gray-600 mb-4">Completa esto porque marcaste que la unidad se atiende fuera del taller.</p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-medium text-gray-400 mb-1.5">¿Por qué se envía fuera del taller?</label>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        {{-- ============ TAB 3: Refacciones y evidencia ============ --}}
        <div x-show="activeTab === 'evidencia'" x-cloak class="space-y-4">
            ...
            <div class="bg-gray-800/40 border border-gray-700/40 rounded-2xl p-5">
                ...
                @else
                    <p class="text-xs text-gray-600">Guarda el avance en la pestaña "Diagnóstico y atención" para poder marcar el reporte como completado.</p>
                @endif
            </div>
        </div>
